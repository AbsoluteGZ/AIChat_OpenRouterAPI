...
class ChatApp:
    ...
    def __init__(self):
        ...
        self.api_key = None  # Для хранения API ключа
        self.pin = None      # Для хранения PIN-кода
    ...
    async def authenticate(self, page: ft.Page):
        async def on_auth_complete(key, pin):
            if key and pin:
                self.api_key = key
                self.pin = pin
                self.api_client.api_key = key # Обновляем ключ в клиенте API
                self.api_client.headers["Authorization"] = f"Bearer {key}"
                self.cache.set_api_key(key)
                self.cache.set_pin(pin)
                self.update_balance()
                page.add(self.main_column)  # Добавляем основной интерфейс после аутентификации
                page.update()

        auth_dialog = AuthDialog(on_auth_complete, self.cache)
        await auth_dialog.show(page)   
    ...
    async def main(self, page: ft.Page):
        ...
        if not self.cache.get_setting("api_key"):
            page.add(ft.ProgressRing())
            page.update()
            await self.authenticate(page) 

        else:
            self.api_key = self.cache.get_setting("api_key")
            self.pin = self.cache.get_setting("pin")
            self.api_client.api_key = self.api_key
            self.api_client.headers["Authorization"] = f"Bearer {self.api_key}"
            self.update_balance()
