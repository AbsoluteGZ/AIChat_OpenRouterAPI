...
class ChatCache:
    ...
    def create_tables(self):
        ...
        cursor.execute(''' -- Создаем таблицу для настроек
    ...
    def clear_all(self): # Очищаем все, включая ключ и PIN
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('DELETE FROM messages')
        cursor.execute('DELETE FROM analytics_messages')
        cursor.execute('DELETE FROM settings') # Очищаем таблицу настроек
        conn.commit()
