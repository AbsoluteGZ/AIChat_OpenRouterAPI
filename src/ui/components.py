...
class AuthDialog:
    def __init__(self, on_complete, cache):
        ...
        self.dlg = ft.AlertDialog(
            ...
            content=ft.Column([
                ft.TextField(label="OpenRouter API Key", value=self.cache.get_setting("api_key") or "", password=True),
            ]),
            actions=[
                ft.TextButton("OK", on_click=self.auth_ok),
    ...
    async def auth_ok(self, e):
        key_field = self.dlg.content.controls[0]  # Получаем поле ввода ключа
        ...
        try:
            if api_key:
                ...
                if balance.startswith("$") and float(balance[1:]) > 0:
                    if not self.cache.get_pin():  # Генерируем PIN только при первом входе
                        pin = str(random.randint(1000, 9999))
                    else:
                        pin = self.cache.get_pin()
                    self.dlg.open = False
                    e.page.overlay.remove(self.dlg)
                    await self.on_complete(api_key, pin)
                else:
                    self.dlg.content.controls.append(ft.Text("Недостаточно средств или неверный ключ", color=ft.colors.RED))
                    e.page.update()

        except Exception as e:
            self.dlg.content.controls.append(ft.Text(str(e), color=ft.colors.RED))
            e.page.update()

    async def reset_key(self, e):
        self.cache.clear_all() # Очищаем все данные из кеша
        self.dlg.content.controls[0].value = ""  # Очищаем поле ввода ключа
        self.dlg.content.controls.append(ft.Text("Ключ сброшен. Пожалуйста введите новый ключ.", color=ft.colors.YELLOW_700))
